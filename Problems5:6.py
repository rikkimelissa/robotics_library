...
for i in range(100):
    ...
    pDlist[i] = p
    rDlist[i] = r
    th = IKinBodyMat(screw_axes_ur5_B,M_ur5,Xlist[i+1],thStart,.0017,.00001,100)
    print th
    thlist[i+1] = IKinBody(screw_axes_ur5_B,M_ur5,Xlist[i+1],thStart,.0017,.00001,100)[0]
...
pylab.plot(tSpace[0:100],pDlist[:,0],'-r',label='Joint 0')
pylab.plot(tSpace[0:100],pDlist[:,1],'-g',label='Joint 1')
pylab.plot(tSpace[0:100],pDlist[:,2],'-b',label='Joint 2')
pylab.plot(tSpace[0:100],pAlist[:,0],'--r',label='Joint 0')
pylab.plot(tSpace[0:100],pAlist[:,1],'--g',label='Joint 1')
pylab.plot(tSpace[0:100],pAlist[:,2],'--b',label='Joint 2')
pylab.xlabel('Time (s)')
pylab.ylabel('Joint position (rad)')
pylab.title('Joint Trajectory')
pylab.legend(loc='upper left')
